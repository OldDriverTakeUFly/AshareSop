...
The four modules are production code in `stockhot/limit_up`, `stockhot/dragon_tiger`, `stockhot/fund_flow`, and `stockhot/risk_alert`. This skill is an orchestration wrapper. It references these modules, never modifies their source. Two additional Wave-2 modules (`stockhot/index_technical`, `stockhot/volatility`) extend the scan with technical-trend and volatility/sentiment dimensions.
...
### Module E: 波动率观察 (volatility) — Wave 2 扩展
...
**Role in scan:** Wave 2 扩展模块，2026-07-06 新增。独立无依赖（读指数日线 + iVIX，不读任何上游 DB 数据）。内部已 `save_daily_data`，供盘后总结（情绪温度计）/盘前报告（风控检查）消费。失败不影响其他模块。
...
+---------+     +-----------------+                   |    +------------+
                ...
                +-----------------+                   |
                +-----------------+                   |
                | index_technical | (独立，不喂 risk_alert)
                +-----------------+
                +-----------------+
                | volatility      | (独立，不喂 risk_alert)
                +-----------------+
```
...
| 2 | `dragon_tiger` + `fund_flow` + `index_technical` + `volatility` (parallel) | 四者互不依赖。dragon_tiger 是 limit_up 的逻辑下游（同一批热点股的席位层面）；fund_flow 独立；`index_technical` 独立（采集指数 OHLCV + 6 阶段趋势识别）；`volatility` 独立（采集指数日线 + iVIX 历史，计算 RV 分位 + V/R 比率）。四者都写 DB 供 risk_alert / 盘后总结 / 盘前报告消费。 |
...
# Wave 2: dragon_tiger + fund_flow + index_technical + volatility in parallel
try:
...
try:
    results["volatility"] = run_volatility_analysis(date)  # 内部已 save_daily_data
except Exception as e:
    results["volatility"] = {"date": date, "status": "数据不可用", "error": str(e)}
