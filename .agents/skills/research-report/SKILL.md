...
24. **生成音频版**（可选但推荐）——用 `report_to_audio.py` 将研报转为 MP3 音频，方便通勤/运动时听：
    ```bash
    .venv/bin/python scripts/report_to_audio.py "docs/个股研报/.../某某深度研报.md"
    ```
    脚本默认女声晓晓（zh-CN-XiaoxiaoNeural），语速 +10%，自动输出到 `docs_audio/` 镜像目录（保持与 `docs/` 相同的子目录结构）。MP3 文件不提交 git（已在 .gitignore），仅在本地使用。
