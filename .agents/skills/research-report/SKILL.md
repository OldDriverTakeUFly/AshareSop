...
8. **估值分析**：
   ...
   - **股息可持续性校验（强制，凡报告或结论涉及股息率时）**：TTM 股息率（`dv_ttm`）是"去年分红÷今日股价"的滞后口径，**禁止单独作为投资依据**。必须做三重校验并同时给出 TTM 与 forward 两个口径：
     1. **分红历史**：`pro.dividend`（div_proc="实施"）取近 3-5 年每股派息——识别一次性特别分红（信号：分红率>100%、或 DPS 较上年跳升 2 倍以上、或一年内多次实施含年报外分红）；
     2. **分红率**：最新年度 DPS/EPS——可持续区间 30-80%；>90% 警惕（盈利下滑时必然削减）；
     3. **业绩预告方向**：`pro.forecast` 预减/首亏时，forward 股息率 ≈ TTM × (1+盈利预期变动)（假设分红率不变），首亏年度分红大概率暂停或腰斩。
     实证样本（2026-08）：牧原 TTM 3.4% 但首亏→分红暂停；格力 2025 年度 DPS 从 3.0 缩至 1.0；塔牌 H1 预减 50%→forward 股息约腰斩；荣晟环保/达仁堂 DPS 跳升 3-4 倍系一次性特别分红，TTM 14%/13% 完全失真。
...
12. **按类型模板写正文**（读 `references/report-templates.md`）。表格化呈现（每小节至少一个数据表）。核心洞察用 `> **核心洞察**：...` 引用块。
    ...
    **个股研报的三个"灵魂章节"必须详尽（参考德福科技黄金标准 `docs/研报/个股/半导体电子/德福科技深度研报.md`）：**
...
24. **语音化**（可选）——md 报告是给「看」设计的，表格/ASCII 框图/百分位数字直接朗读无法听懂。可转口语化文本辅助阅读：
    ...
    **转口语化文本**——把表格转成"净利润走过一个完整周期，从 0.2 亿反弹至 1.95 亿又跌回 0.6 亿"这样的叙述，而非逐行流水账：
    ```bash
    .venv/bin/python scripts/tools/report_to_speech_text.py "docs/研报/个股/.../某某深度研报.md"
    ```
    ...
    > ⏸️ **MP3 音频输出已暂停**（2026-08-09）：`scripts/tools/report_to_audio.py`（edge-tts 合成）实用价值有限且生成时占用大量 CPU，暂时从流程中移除。如需恢复，重新调用该脚本即可，输出到 `docs_audio/`（MP3 不提交 git）。
...
- 方法论参考：`docs/研报/方法论/`（9 篇方法论文档，写作时引用）
...
如果本 skill 与实际报告产物（`docs/` 里的黄金标准）冲突，以实际产物为准。方法论细节以 `docs/研报/方法论/` 为准；估值/景气度计算以 `davis_analyzer` 引擎为准。
