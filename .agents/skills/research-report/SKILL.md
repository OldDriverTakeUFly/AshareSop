...
4. **结构化数据**（调引擎，不手算）：
   - 财务数据：`davis_analyzer.tushare_client` 取 income/balancesheet/cashflow/fina_indicator
   - 估值分位：`davis_analyzer.valuation` 计算 PE/PB 3 年百分位
   - 景气度：`davis_analyzer.prosperity` 计算 G+ΔG + DuPont
   - 困境反转：`davis_analyzer.distress` 计算 3 层 9 信号
   - 综合评分：`davis_analyzer.scoring` 计算 Davis Double 加权分
