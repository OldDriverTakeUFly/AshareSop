...
| **`pro.top10_floatholders` 的 `ratio` 列与 `pro.dividend` 的 `cash_div_taxable` 列被新端点裁剪（KeyError）** | 同为 dataapi 字段裁剪家族：top10_floatholders 返回不带 ratio（无法算十大流通股东合计持股比例）、dividend 返回不带 cash_div_taxable/div_proc 等明细列（三安研报 2026-08 首次踩到，脚本直接崩） | 两处取数后先判 `列 in df.columns` 再访问，缺失时置 None/标注"端点字段裁剪不可用"并在报告诚实声明，勿硬取；十大流通股东交叉验证可跳过（stk_holdernumber 已够用），分红明细改用年报口径 |
