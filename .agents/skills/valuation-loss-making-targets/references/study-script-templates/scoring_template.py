#!/usr/bin/env python3
"""单股困境反转戴维斯双击评分模板脚本 (通用版).

这是一个参数化模板：复制此文件后，仅需修改顶部 CONFIG 块即可对任意 A 股标的
执行完整四维评分。本脚本只读调用 davis_analyzer 核心函数，不修改任何源码。

评分维度:
    1. 估值层 — PE/PB 3年历史百分位 (valuation.py: calculate_valuation_score)
    2. 趋势层 — PE/PB 月度趋势斜率/加速度 (trend.py: batch_trend / calculate_trend_slope)
    3. 景气层 — 营收/利润/斜率/持续时间四维复合 (prosperity.py: calculate_prosperity_score)
    4. 困境层 — 三层信号：困境确认 + 反转可能 + 反转激活 (distress.py: calculate_distress_score)

最终通过 scoring.py 的 calculate_davis_double_score() 合成戴维斯双击综合评分。

────────────────────────────────────────────────────────────────────────────────
【必填配置】
    打开本文件，修改 CONFIG 块中的三个变量：
...
【用法】
    # 1. 复制并修改 CONFIG
    cp scoring_template.py my_stock_scoring.py
    # 编辑 my_stock_scoring.py 的 CONFIG 块

    # 2. 运行 (需在仓库根目录，确保 davis_analyzer 可被导入)
    .venv/bin/python skills/.../my_stock_scoring.py

【输出】
    JSON 文件写入 OUTPUT_DIR/{TARGET_CODE}_davis_score.json，
...
# ── 派生常量 (请勿手动修改) ──
PERIODS = 12
_OUTPUT_PATH = Path(OUTPUT_DIR) / f"{TARGET_CODE}_davis_score.json"


# ════════════════════════════════════════════════════════════════════════════
# 信号得分依据生成器 — 解释每个信号为什么得这个分
#
# 以下 _explain_* 函数调用 distress.py 的单个 check_* 子函数重新计算连续得分，
# 并返回人类可读的中文解释字符串，写入最终 JSON 的 signals_detail.<layer>.<signal>.reason
# 字段。参数均为已从财务/估值数据提取出的标量或历史序列，不含任何股票特定信息。
# ════════════════════════════════════════════════════════════════════════════
...
def _explain_eps_decline(eps_history: list[float]) -> str:
    ...
    输入: EPS 历史序列（最新在前，降序）。
    引擎: distress.check_eps_decline — 若最新 EPS 同比下滑 ≥30% 则满档，否则线性折算。
    """
...
def _explain_pe_pb_percentile(pe_pct: float, pb_pct: float) -> str:
    ...
    输入: PE/PB 在3年历史中的百分位 (0.0-1.0)。
    引擎: distress.check_pe_pb_percentile — 得分 = 1.0 - (pe_pct+pb_pct)/2。
    """
...
def _explain_financial_health(debt_ratio: float, operating_cf: float) -> str:
    ...
    输入: 资产负债率 (0-1), 经营现金流（元）。
    引擎: distress.check_financial_health — 负债率<50%得0.5，经营CF>0得0.5，二者相加。
    """
...
def _explain_balance_sheet(total_debt: float, total_assets: float) -> str:
    ...
    输入: 总负债、总资产（元）。
    引擎: distress.check_balance_sheet — 得分 = max(0, 1 - 负债率×2)。
    """
...
def _explain_operating_cf(operating_cf: float, total_assets: float) -> str:
    ...
    输入: 经营现金流、总资产（元）。
    引擎: distress.check_operating_cf — 得分 = clamp(经营CF/总资产, 0, 1)。
    """
...
def _explain_roe_trend(roe_history: list[float]) -> str:
    ...
    输入: ROE 历史序列（最新在前，降序，单位 %）。
    引擎: distress.check_roe_trend — 最新ROE较同比/上期改善幅度，≥5个百分点满档。
    """
...
def _explain_revenue_inflection(revenue_history: list[float]) -> str:
    ...
    输入: 营收同比增速历史序列（最新在前，降序，小数）。
    引擎: distress.check_revenue_inflection — clamp((最新-上期)/0.20, 0, 1)。
    """
...
def _explain_profit_inflection(profit_history: list[float]) -> str:
    ...
    输入: 归母净利润同比增速历史序列（最新在前，降序，小数）。
    引擎: distress.check_profit_inflection — clamp((最新-上期)/0.20, 0, 1)。
    """
...
def _explain_delta_g_positive(delta_g: float) -> str:
    ...
    输入: delta_G（近三季度营收增速均值的边际变化，百分点，由 prosperity 引擎计算）。
    引擎: distress.check_delta_g_positive — clamp(delta_G/0.10, 0, 1)。
    """
...
def _build_stock_info(client: TushareClient, ts_code: str, name: str) -> StockInfo:
    ...
    输入: 已初始化的 TushareClient, 目标 ts_code, 默认公司名称。
    输出: StockInfo（含 ts_code/name/industry/list_status/is_cyclical）。
    行业字段用于 valuation/trend 引擎判断是否为周期股（周期股用 PB 替代 PE 主权重）。
    """
    try:
...
def score_stock() -> dict:
    """对 CONFIG 指定的标的执行完整四维评分，返回 JSON 可序列化的 dict.

    流程:
        Step1  初始化 TushareClient
        Step2  fetch_financial_data → 3年(12季度)财务数据
        Step3  提取 EPS/ROE/营收增速/利润增速历史序列 + 最新资产负债表科目
        Step4  fetch_valuation_history + calculate_valuation_score → 估值层得分
        Step5  calculate_prosperity_score → 景气层复合得分
        Step6  batch_trend + calculate_trend_slope/acceleration → 趋势层得分
        Step7  calculate_distress_score → 困境层三层信号得分
        Step8  calculate_davis_double_score → 戴维斯双击综合得分
        Step9  _explain_* 重新解释每个信号得分依据
        Step10 组装最终 JSON dict
    """
    ...
    logger.info("=" * 70)
    logger.info("{} ({}) 戴维斯双击困境反转评分", stock_name, ts_code)
    logger.info("=" * 70)
    ...
    # ── Step 3: 提取历史序列（照抄 pipeline.py 模式）──
    ...
    # 引擎: valuation.fetch_valuation_history → 3年日线 PE_TTM/PB 序列
    #       valuation.calculate_valuation_score → 综合 PE/PB 百分位 (周期股切换权重)
    ...
    #   输出: 复合得分 = revenue*0.30 + profit*0.30 + slope*0.25 + duration*0.15
    #         以及 delta_g（近三季度营收增速均值的边际变化）
    ...
    # ── Step 6: 计算趋势评分 ──
    # 引擎: trend.batch_trend（与 pipeline 一致）+ calculate_trend_slope/acceleration
    #   输入: 日线 PE/PB pd.Series (datetime index) + StockInfo (用于周期股判断)
    #   输出: 趋势评分 (0-100)，月度斜率/加速度用于解释
    ...
    # ── Step 7: 计算困境反转三层评分 ──
    # 引擎: distress.calculate_distress_score
    #   输入: eps/roe/revenue/profit 历史序列, pe/pb 百分位, 资产负债表科目, delta_g
    #   输出: DistressSignal {layer1, layer2, layer3, total}
    ...
    # ── Step 8: 计算戴维斯双击综合评分 ──
    # 引擎: scoring.calculate_davis_double_score
    #   输入: 估值/景气/困境/趋势 四个子分
    #   输出: DavisDoubleScore.final_score = 估值*0.30 + 趋势*0.15 + 景气*0.30 + 困境*0.25
    ...
    # 重新调用每个 check_* 子函数生成人类可读的得分依据，附加到 JSON 输出
...
def main() -> None:
    ...
    输出位置由 OUTPUT_DIR (CONFIG) 决定，文件名为 {TARGET_CODE}_davis_score.json。
    同时将完整 JSON 打印到 stdout。
    """
    result = score_stock()
