...
每日复盘卡(2026-09-01,2026-09-02扩入发布规划):连板天梯/龙虎榜——`scripts/daily_market_cards.py --type {ladder,lhb,all} [--enqueue]` 采集+生成+渲染+入池(生成逻辑在 davis_analyzer/cardgen/daily.py,发稿文案在 daily.publish_copy 固定模板,零数字零敏感词由测试锁定),工作日 17:50 cron 自带采集无人值守,龙虎榜未披露则等≤40分钟;入池≠发布,发布永远人工;零观点纯数据叙事,facts source.kind=stockhot 指纹溯源,expires=当日(过时不发);转债混榜(名字「转N」/代码11x/12x段)已过滤。**废稿池约定(2026-09-02)**:过期未发工程终态归档于 `docs/小红书卡片/废稿/`(手工挪入,sync/归位逻辑跳过该文件夹,台账留档;需复活手工挪回未发布/);发稿池死行(非 published 且过 release_expires 的行)连同 publish_log 挪入同构库 `storage/database/content_publisher_archive.db`,活池只留可发内容——每交易日 23:40 日卡 cron 顺手执行,queue.py 状态机为闭合集不可新增「废稿」状态(勿改 content_publisher)。
