#!/usr/bin/env python3
"""佰维存储 (688525.SH) 单股戴维斯双击评分脚本（存储模组+自封测一体化个股研报取数）.

调用 davis_analyzer 核心函数（不改任何源码），对佰维存储执行完整四维评分，
并附加研报强制数据采集（股东户数 / 5 因子 / 相对估值 / 时效校验 / 3 年分位）。

用法:
    .venv/bin/python davis_analyzer/studies/biwin_scoring.py

输出:
    .sisyphus/evidence/biwin/biwin-davis-score.json
    .sisyphus/evidence/biwin/biwin-extended.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/biwin/biwin-davis-score.json")
EXTENDED_PATH = Path(".sisyphus/evidence/biwin/biwin-extended.json")
...
def collect_extended() -> dict:
    ...
    # ── 10. 资产负债表 + 现金流量表（存货/OCF 关键科目，report 口径）──
    try:
        bal = pro.balancesheet(ts_code=TS_CODE, period_type="report",
                               fields="ts_code,end_date,ann_date,inventory,inventories,total_assets,total_liabilities")
        bal = bal.sort_values("end_date")
        rows_b = []
        for _, r in bal.iterrows():
            inv = r.get("inventory") if "inventory" in bal.columns else None
            if inv is None or (isinstance(inv, float) and pd.isna(inv)):
                inv = r.get("inventories") if "inventories" in bal.columns else None
            rows_b.append({
                "end_date": str(r["end_date"]),
                "inventory_yi": round(float(inv) / 1e8, 2) if inv is not None and not (isinstance(inv, float) and pd.isna(inv)) else None,
                "total_assets_yi": round(float(r["total_assets"]) / 1e8, 2) if pd.notna(r.get("total_assets")) else None,
                "total_liab_yi": round(float(r["total_liabilities"]) / 1e8, 2) if pd.notna(r.get("total_liabilities")) else None,
            })
        ext["balancesheet"] = rows_b
    except Exception as e:  # noqa: BLE001
        ext["balancesheet_error"] = str(e)

    try:
        cf = pro.cashflow(ts_code=TS_CODE, period_type="report", fields="ts_code,end_date,n_cashflow_act")
        cf = cf.sort_values("end_date")
        ext["cashflow_act"] = [
            {"end_date": str(r["end_date"]), "ocf_yi": round(float(r["n_cashflow_act"]) / 1e8, 2) if pd.notna(r["n_cashflow_act"]) else None}
            for _, r in cf.iterrows()
        ]
    except Exception as e:  # noqa: BLE001
        ext["cashflow_error"] = str(e)
