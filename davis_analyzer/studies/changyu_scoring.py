#!/usr/bin/env python3
"""长裕集团 (603407.SH) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对长裕集团执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/changyu_scoring.py

输出:
    .sisyphus/evidence/changyu/t2-davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/changyu/t2-davis-score.json")
...
def score_changyu() -> dict:
    """执行长裕集团完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("长裕集团 (603407.SH) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
