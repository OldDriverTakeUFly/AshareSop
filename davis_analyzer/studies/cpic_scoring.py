#!/usr/bin/env python3
"""国际复材 (301526.SZ) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对国际复材执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/cpic_scoring.py

输出:
    .sisyphus/evidence/cpic/c2-davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/cpic/c2-davis-score.json")
...
def score_cpic() -> dict:
    """执行国际复材完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("国际复材 (301526.SZ) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
