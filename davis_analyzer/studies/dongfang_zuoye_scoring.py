#!/usr/bin/env python3
"""东方锆业 (002167.SZ) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对东方锆业执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/dongfang_zuoye_scoring.py

输出:
    .sisyphus/evidence/dongfang_zuoye/t2-davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/dongfang_zuoye/t2-davis-score.json")
...
def score_dongfang_zuoye() -> dict:
    """执行东方锆业完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("东方锆业 (002167.SZ) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
