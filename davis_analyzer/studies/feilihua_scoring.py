#!/usr/bin/env python3
"""菲利华 (300395.SZ) v3.0 数据刷新评分脚本（研报版本更新专用）.

基于 guoci_scoring.py 模板改造（石英布 AI 涨价逻辑融入研报 v3.0 的数据底座）：
  ...
  - 行情复核（20260731 v2.0 快照 → 20260814 区间收益，复权口径；AI 涨价链同行对照另见研报）
  - 股东户数趋势（stk_holdernumber 直连，dropna 防 NaN 垃圾行）

用法（必须从仓库根目录运行）:
    cd /home/leo/Projects/CodeAgentDashboard
    .venv/bin/python davis_analyzer/studies/feilihua_scoring.py

输出:
    davis_analyzer/studies/output/feilihua_v3_20260814.json
"""
...
PERIODS = 12
ASOF = "20260814"
OUTPUT_PATH = Path(f"{os_proj_root}/davis_analyzer/studies/output/feilihua_v3_{ASOF}.json")
...
def manual_returns(pro) -> dict:
    """复权口径手工复核多窗口收益（v2.0 快照 20260731 → 20260814、60/120/250d）."""
    end = datetime.strptime(ASOF, "%Y%m%d").date()
    ...
    # 年初至今 + v2.0 快照锚点对照
    for anchor in ("20260101", "20260708", "20260717", "20260731", "20260801"):
...
def main() -> None:
    ...
    result["valuation"] = {
        ...
        "snapshot_20260731": {
            ...
            if row["trade_date"] in ("20260708", "20260717", "20260731")
        },
    }
    logger.info(
    ...
    logger.info(
        ...
        hc.concentration_score if hc is not None else None,
        pq.quality_score,
    )
