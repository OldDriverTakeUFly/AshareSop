#!/usr/bin/env python3
"""兆易创新 (603986.SH) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对兆易创新执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/giga_scoring.py

输出:
    .sisyphus/evidence/giga/g2-davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/giga/g2-davis-score.json")
...
def score_giga() -> dict:
    """执行兆易创新完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("兆易创新 (603986.SH) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
    ...
    # 已知坑点：fetch_valuation_history 可能只返回 ~22 天（SQLite 增量 fetch 未命中），
    # 不足 3 年窗口。此时用 pro.daily_basic 分段直连拉全量，手工构造 ValuationData。
    if val_history and len(val_history) < 700:
        logger.warning(
            "估值历史仅 {} 天（<700），改用 pro.daily_basic 分段直连拉 3 年全量",
            len(val_history),
        )
        ...
        try:
            ...
            frames = []
            end_d = date.today()
            seg_start = end_d - timedelta(days=1095)
            while seg_start < end_d:
                seg_end = min(seg_start + timedelta(days=480), end_d)
                df = pro.daily_basic(
                    ts_code=TS_CODE,
            ...
            full = (
                ...
                .dropna(subset=["pe_ttm", "pb"], how="any")
                .drop_duplicates("trade_date")
                .sort_values("trade_date", ascending=False)
            )
            logger.info("直连拉取到 {} 个交易日估值数据", len(full))
            if len(full) > len(val_history):
                val_history = [
                    ValuationData(
                        ts_code=r["ts_code"],
                        ...
                        pb=float(r["pb"]),
                        ps=float(r.get("ps") if pd.notna(r.get("ps")) else 0.0),
                        total_mv=float(r.get("total_mv") if pd.notna(r.get("total_mv")) else 0.0),
                    )
                    for _, r in full.iterrows()
                ]
        except Exception:
            logger.exception("直连拉取估值历史失败，沿用原 val_history")
    if not val_history:
