#!/usr/bin/env python3
"""宏和科技 (603256.SH) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对宏和科技执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/honghe_scoring.py

输出:
    .sisyphus/evidence/honghe/t2-davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/honghe/t2-davis-score.json")
...
def score_honghe() -> dict:
    """执行宏和科技完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("宏和科技 (603256.SH) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
