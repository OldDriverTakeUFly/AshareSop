#!/usr/bin/env python3
"""嘉元科技 (688388.SH) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对嘉元科技执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/jiayuan_scoring.py

输出:
    .sisyphus/evidence/jiayuan/t2-davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/jiayuan/t2-davis-score.json")
...
def score_jiayuan() -> dict:
    """执行嘉元科技完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("嘉元科技 (688388.SH) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
