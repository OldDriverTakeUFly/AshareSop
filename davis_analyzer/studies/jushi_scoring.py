#!/usr/bin/env python3
"""中国巨石 (600176.SH) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对中国巨石执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/jushi_scoring.py

输出:
    .sisyphus/evidence/jushi/t2-davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/jushi/t2-davis-score.json")
...
def score_jushi() -> dict:
    """执行中国巨石完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("中国巨石 (600176.SH) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
