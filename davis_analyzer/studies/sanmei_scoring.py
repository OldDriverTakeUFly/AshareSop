#!/usr/bin/env python3
"""三美股份 (603379.SH) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对三美股份执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/sanmei_scoring.py

输出:
    .sisyphus/evidence/sanmei/t2-davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/sanmei/t2-davis-score.json")
...
def score_tianyue() -> dict:
    """执行三美股份完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("三美股份 (603379.SH) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
