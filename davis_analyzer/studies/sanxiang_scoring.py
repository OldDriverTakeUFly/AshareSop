#!/usr/bin/env python3
"""三祥新材 (603663.SH) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对三祥新材执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/sanxiang_scoring.py

输出:
    .sisyphus/evidence/sanxiang/t2-davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/sanxiang/t2-davis-score.json")
...
def score_sanxiang() -> dict:
    """执行三祥新材完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("三祥新材 (603663.SH) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
