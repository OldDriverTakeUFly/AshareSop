#!/usr/bin/env python3
"""圣泉集团 (605589.SH) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对圣泉集团执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/shengquan_scoring.py

输出:
    .sisyphus/evidence/shengquan/t2-davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/shengquan/t2-davis-score.json")
...
def score_shengquan() -> dict:
    """执行圣泉集团完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("圣泉集团 (605589.SH) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
