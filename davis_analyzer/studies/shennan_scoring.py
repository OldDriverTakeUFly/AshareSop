#!/usr/bin/env python3
"""深南电路 (002916.SZ) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对深南电路执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/shennan_scoring.py

输出:
    .sisyphus/evidence/shennan/t2-davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/shennan/t2-davis-score.json")
...
def score_shennan() -> dict:
    """执行深南电路完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("深南电路 (002916.SZ) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
