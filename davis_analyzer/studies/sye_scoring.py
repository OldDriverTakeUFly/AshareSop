#!/usr/bin/env python3
"""生益电子 (688183.SH) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对生益电子执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/sye_scoring.py

输出:
    .sisyphus/evidence/sye/t2-davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/sye/t2-davis-score.json")
...
def score_sye() -> dict:
    """执行生益电子完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("生益电子 (688183.SH) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
