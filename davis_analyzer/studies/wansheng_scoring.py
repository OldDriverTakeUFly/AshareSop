#!/usr/bin/env python3
"""万盛股份 (603010.SH) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对万盛股份执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/wansheng_scoring.py

输出:
    .sisyphus/evidence/wansheng/davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/wansheng/davis-score.json")
...
def score_wansheng() -> dict:
    """执行万盛股份完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("万盛股份 (603010.SH) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
