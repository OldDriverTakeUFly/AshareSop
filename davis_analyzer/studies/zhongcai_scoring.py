#!/usr/bin/env python3
"""中材科技 (002080.SZ) 单股困境反转评分脚本.

调用 davis_analyzer 核心函数（不改任何源码），对中材科技执行完整四维评分：
...
用法:
    .venv/bin/python davis_analyzer/studies/zhongcai_scoring.py

输出:
    .sisyphus/evidence/zhongcai/t2-davis-score.json
"""
...
OUTPUT_PATH = Path(".sisyphus/evidence/zhongcai/t2-davis-score.json")
...
def score_zhongcai() -> dict:
    """执行中材科技完整四维评分，返回 JSON 可序列化的 dict."""

    logger.info("=" * 70)
    logger.info("中材科技 (002080.SZ) 戴维斯双击困境反转评分")
    logger.info("=" * 70)
