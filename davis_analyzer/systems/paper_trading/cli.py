...
Usage::
    ...
    python -m davis_analyzer.systems.paper_trading init --name davis_v1 \\
        --strategy davis_double --capital 1000000

    # Run one trading day (live mode)
    python -m davis_analyzer.systems.paper_trading run --name davis_v1

    # Backfill from a historical start date
    python -m davis_analyzer.systems.paper_trading backfill --name davis_v1 \\
        --start 20260101

    # Generate performance report
    python -m davis_analyzer.systems.paper_trading report --name davis_v1

    # List all accounts
    python -m davis_analyzer.systems.paper_trading list
"""
...
def cmd_init(args):
    ...
    from davis_analyzer.systems.paper_trading.strategy import STRATEGY_REGISTRY
    ...
    try:
        ...
        print(f"\n  使用 `python -m davis_analyzer.systems.paper_trading run --name {args.name}` 开始交易")
...
def main():
    parser = argparse.ArgumentParser(
        prog="davis_analyzer.systems.paper_trading",
