...
"""recap CLI:python -m davis_analyzer.systems.recap {run|select|script|sheet|audio|post|status}。"""
...
from davis_analyzer.systems.recap import data, recorder_sheet as sheet, scriptwriter  # noqa: F401 (测试 monkeypatch 锚点)
...
def _do_select(day: str) -> list[Candidate]:
    from davis_analyzer.systems.recap import db, selector
...
def cmd_audio(args) -> None:   # Task 8 实现 + Task 10 视觉质检闸
    from davis_analyzer.systems.recap.audio_pack import make_pack
    from davis_analyzer.systems.recap import card_renderer, vision_qc
...
def cmd_post(args) -> None:    # 二期:ffmpeg 合成 + 成片抽帧视觉质检 + 台账 composed
    ...
    from davis_analyzer.systems.recap import db, vision_qc
    from davis_analyzer.systems.recap.post_compose import compose
    from davis_analyzer.systems.cardgen.video import audio_duration, ffmpeg
