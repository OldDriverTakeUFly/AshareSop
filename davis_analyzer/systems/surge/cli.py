"""surge CLI: run / backfill / status (python -m davis_analyzer.systems.surge)."""
...
from davis_analyzer.systems.surge import chips, cninfo, db, report, screen
...
def _cmd_run(args: argparse.Namespace) -> int:
    ...
    if not args.no_longpic:
        from davis_analyzer.systems.surge import longpic
        conn = db.connect()
    ...
    if not args.no_push:
        from davis_analyzer.systems.surge.push_feishu import push_day
        push_day(out["day"])
    return 0
