...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "list":
        from davis_analyzer.systems.tournament.adapters import default_participants
        for p in default_participants():
            print(f"参赛者: {p.name:<24} horizon={p.horizon:<8} version={p.version}")
        return 0
    if args.command == "run":
        from datetime import datetime
        from davis_analyzer.systems.tournament.adapters import default_participants, resolve_universe
        from davis_analyzer.systems.tournament.judge import JudgeHarness, trading_calendar
        from davis_analyzer.systems.tournament.report import render_report, write_report
        from davis_analyzer.systems.tournament.scorecard import score_participant
    ...
    if args.command == "replay":
        ...
        from davis_analyzer.systems.tournament.adapters import default_participants, resolve_universe
        from davis_analyzer.systems.tournament.judge import JudgeHarness, trading_calendar
        from davis_analyzer.systems.tournament.ledger import LedgerRecord, append_record, open_db
        from davis_analyzer.systems.tournament.replay import export_replay, replay
        from davis_analyzer.core.tushare_client import TushareClient
    ...
    if args.command == "champions":
        ...
        from davis_analyzer.systems.tournament.ledger import open_db

        conn = open_db()
        from davis_analyzer.systems.tournament.champions import ensure_tables as ensure_ch
        ensure_ch(conn)
