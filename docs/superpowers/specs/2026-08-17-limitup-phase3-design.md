...
5. **次日执行提示**（§3.2.2 盘中纪律的每候选定制版）：若次日开盘=涨停价→持有观察+炸板即卖；低开走弱→开盘卖。
6. 免责口径声明：基于日线近似（EOD 特征做当日决策），与回测同口径。
...
### 3.2.1 卖出侧规则矩阵（2026-08-18 分桶研究结论，75,998 个首板事件）

T+1 形态分桶 ×（开盘卖 vs 持有到 T+2 开盘）期望：

| T+1 情形 | 占比 | T+1 开盘卖 | 持有到 T+2 开盘 | 持有更优占比 | 结论 |
|---|---|---|---|---|---|
| A 一字涨停（全天锁死） | 3.1% | +10.53% | **+18.90%** | 90.8% | 持有显著占优 |
| B 高开上板（非一字） | 13.6% | +5.73% | **+13.52%** | 87.3% | 持有显著占优 |
| C 低开上板（非一字） | 1.2% | -1.81% | **+12.83%** | 99.8% | 持有大幅占优 |
| D 高开断板 | 54.8% | **+3.06%** | +0.06% | 26.4% | 开盘卖显著占优 |
| E 低开断板 | 27.3% | **-2.04%** | -3.10% | 39.0% | 开盘卖占优 |

由此确定的卖出规则矩阵：

1. **基准 = `open_next`（无条件 T+1 开盘卖）**。维持不变。
2. **全量条件持有 `ride_board` 已证伪**（first_board × ride_board 对照回测：胜率 68%→39%、回撤 -4.5%→-56.6%、悲观档转负——断板股 82% 被迫多持一天吃满回落）。不采纳。
3. **理想规则「上板持有、断板开盘卖」每股期望约 +3.7%（vs 基准 +2.2%）**，但 B/C 桶的判定时点在盘中（快速上板），9:25 与盘后均不可复现——**进执行手册，不进机器规则**（写进规则即前视）。
4. **可观测变体 `open_hold_locked`（Phase 3 前置研究项）**：T+1 开盘价=涨停价（9:25 可判定）→ 持有并进入 ride 循环（后续每日收盘涨停持有、断板次日开盘卖）；否则 T+1 开盘卖。实证：整桶（6.4%）持有 +12.70% vs 卖 +10.65%，摊薄后仅 +0.13pp/笔——其中全天锁死子桶强占优、盘中炸杯子桶反向。该变体无前视（决策只用开盘价），列为研究项：回测三档通过且 IS/OOS 一致才考虑替代基准；否则丢弃。

### 3.2.2 盘中执行纪律（写入 candidates 报告的「次日执行提示」，人工执行）

- T+1 开盘涨停且封单大：可持有观察；**一旦炸板立即卖出**（炸杯子桶持有 +7.5% < 卖出 +10.8%，炸板不卖是明确的负期望行为）。
- T+1 高开/低开后盘中快速上板：可持有（B/C 桶持有期望 +12.8%~+13.5%），断板次日开盘卖。
- T+1 低开走弱未上板：开盘直接卖（E 桶开盘卖 -2.04% 已是最优可执行选择）。
- 机器规则（模拟盘）维持 open_next；上述纪律仅作为实盘人执行参考与后续若接入分钟线时的规则蓝本。
