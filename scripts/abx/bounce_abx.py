"""A/B: 超跌反弹子策略的效果验证.
  ...
  O0_baseline  — 当前生产配置（无超跌反弹）
  O1_bounce    — 启用超跌反弹子策略（2 slot）

实证依据:
  市场层: 上证跌>5% + RV5/RV20>0.8 → 20d +4-5% (胜率75-89%)
  个股层: 触发日选跌幅最深 → Q5-Q1 = +4.41%
  卖出:   RV5/RV20 < 0.8 (波动衰减即卖)
"""
...
BASE = dict(
    ...
    low_vol_stop_exemption=0.0, ivix_pause_threshold=0.0,
...
def run_variant(label, enable_bounce, universe):
    print(f"\n{'='*70}\n  {label}: oversold_bounce={enable_bounce}\n{'='*70}", flush=True)

    account = reset_account(f"bounce_{label}")
    params = {**BASE, "enable_oversold_bounce": enable_bounce}
    ...
    # Bounce 交易统计
    bounce_buys = [t for t in trades if "超跌反弹" in (t.signal_reason or "")]
    bounce_sells = [t for t in trades if "超跌反弹卖出" in (t.signal_reason or "")]
    ...
    result = {
        "label": label, "enable_bounce": enable_bounce,
        ...
        "n_bounce_sells": len(bounce_sells),
    ...
    print(f"  → {label}: 毛收益={total_ret:+.2f}% 净={ret_net:+.2f}% "
          ...
          f"交易={len(trades)} (反弹买{len(bounce_buys)}卖{len(bounce_sells)}) "
...
def main():
    ...
    results = []
    results.append(run_variant("O0_baseline", False, universe))
    results.append(run_variant("O1_bounce", True, universe))

    print(f"\n\n{'='*90}")
    print(f"  超跌反弹 A/B 总览 — {START} → {END}")
    print(f"{'='*90}")
    print(f"  {'变体':<16} {'反弹':>5} {'净收益':>8} {'回撤':>6} {'Sharpe':>7} {'交易':>5} {'反弹买':>6}")
    for r in sorted(results, key=lambda x: -x["sharpe_real"]):
        print(f"  {r['label']:<16} {'✅' if r['enable_bounce'] else '❌':>5} "
              f"{r['return_pct_net']:>+7.2f}% {r['max_drawdown_pct']:>5.1f}% "
              f"{r['sharpe_real']:>+7.3f} {r['n_trades']:>5} {r['n_bounce_buys']:>6}")
