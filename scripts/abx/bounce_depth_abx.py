...
全市场实证（66触发日 × 24k样本）:
  跌幅越深反弹越强: -3~-10% → +3.77%, -10~-20% → +4.58%, <-20% → +10.54%
"""
...
def run_variant(label, min_drop, universe):
    print(f"\n{'='*70}\n  {label}: min_drop={min_drop}\n{'='*70}", flush=True)
    account = reset_account(f"depth_{label}")
    params = {**BASE, "oversold_candidate_min_drop": min_drop}
...
def main():
    ...
    results = []
    results.append(run_variant("D0_drop3", -3.0, universe))
    results.append(run_variant("D1_drop10", -10.0, universe))
    results.append(run_variant("D2_drop15", -15.0, universe))

    print(f"\n\n{'='*95}")
    print(f"  反弹跌幅门槛 A/B 总览")
    print(f"{'='*95}")
    print(f"  {'变体':<14} {'门槛':>6} {'收益':>8} {'MDD':>6} {'Sharpe':>7} {'反弹买':>6} {'反弹笔均':>8}")
    for r in sorted(results, key=lambda x: -x["sharpe_real"]):
        print(f"  {r['label']:<14} {r['min_drop']:>5.0f}% {r['return_pct']:>+7.2f}% "
              f"{r['max_drawdown_pct']:>5.1f}% {r['sharpe_real']:>+7.3f} "
              f"{r['n_bounce_buys']:>6} {r['avg_bounce_ret']:>+7.1f}%")
    ...
    out_path = "logs/abx/bounce_depth_abx.json"
