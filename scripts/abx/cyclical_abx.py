"""A/B: 周期股持仓辅助规则效果验证.
  ...
  C0_baseline  — 无周期股规则
  C1_cyclical  — 启用两条规则:
    规则1: 周期股持仓>30天 且 浮盈<3% → 清仓（周期已过）
    规则2: 周期股浮盈>15% → 止损放宽×2（超级周期保护）

实证依据（5年327笔）:
  周期股 31-60天: 平均-1.25%, 胜率33%（死亡区间）
  周期股 60天+:  平均+41.5%（超级周期）
"""
...
def run_variant(label, enable_cyclical, universe):
    print(f"\n{'='*70}\n  {label}: cyclical_rules={enable_cyclical}\n{'='*70}", flush=True)
    account = reset_account(f"cyc_{label}")
    params = {**BASE, "enable_cyclical_rules": enable_cyclical}
    ...
    cyc_exits = [t for t in trades if "周期股清理" in (t.signal_reason or "")]
    ...
    print(f"  → {label}: ret={total_ret:+.2f}% MDD={total_mdd:.1f}% "
          ...
          f"周期清理={len(cyc_exits)} ({elapsed/3600:.1f}h)", flush=True)
...
def main():
    ...
    results = []
    results.append(run_variant("C0_no_cyclical", False, universe))
    results.append(run_variant("C1_cyclical", True, universe))

    print(f"\n\n{'='*90}")
    print(f"  周期股规则 A/B 总览")
    print(f"{'='*90}")
    print(f"  {'变体':<18} {'收益':>8} {'MDD':>6} {'Sharpe':>7} {'交易':>5} {'周期清理':>8}")
    for r in sorted(results, key=lambda x: -x["sharpe_real"]):
        print(f"  {r['label']:<18} {r['return_pct']:>+7.2f}% {r['max_drawdown_pct']:>5.1f}% "
              f"{r['sharpe_real']:>+7.3f} {r['n_trades']:>5} {r['n_cyclical_exits']:>8}")
    ...
    out_path = "logs/abx/cyclical_abx.json"
