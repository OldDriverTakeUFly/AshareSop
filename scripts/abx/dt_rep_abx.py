"""A/B: Dragon-tiger + Repurchase factors combined sweep.

  D0_baseline (reference) — dt=0, rep=0 (current default)
  D1_dt_05               — dragon_tiger_weight=0.05
  D2_rep_05              — repurchase_weight=0.05
  D3_both_05             — dt=0.05 + rep=0.05
"""
...
VARIANTS = [
    ("D0_baseline", {**BASE, "dragon_tiger_weight": 0.0, "repurchase_weight": 0.0}),
    ("D1_dt_05", {**BASE, "dragon_tiger_weight": 0.05, "repurchase_weight": 0.0}),
    ("D2_rep_05", {**BASE, "dragon_tiger_weight": 0.0, "repurchase_weight": 0.05}),
    ("D3_both_05", {**BASE, "dragon_tiger_weight": 0.05, "repurchase_weight": 0.05}),
]
...
def run_variant(label, params, universe):
    dt = params.get("dragon_tiger_weight", 0)
    rep = params.get("repurchase_weight", 0)
    print(f"\n{'='*70}\n  {label}: dt={dt} rep={rep}\n{'='*70}", flush=True)
    account = reset_account(f"dr_{label}", params)
    ...
    account.close()
    return {"label": label, "dt_weight": dt, "rep_weight": rep,
            "return_pct": ret, "max_drawdown_pct": mdd,
...
def main():
    ...
    results = []
    # Reuse D0 from production_final
    try:
        with sqlite3.connect(DB_PATH) as c:
            row = c.execute("SELECT COUNT(*) FROM paper_nav_history n JOIN paper_accounts a ON n.account_id=a.id WHERE a.name='production_final'").fetchone()
            if row[0] >= 120:
                ...
                ret = (nav[-1]/INITIAL_CAPITAL-1)*100 if nav else 0
                mdd = max_dd(nav)
                results.append({"label": "D0_baseline (reused)", "dt_weight": 0, "rep_weight": 0,
                ...
                account.close()
                print("  [reuse] D0 from production_final")
    except: pass
    reused = any("reused" in r["label"] for r in results)
    for label, params in VARIANTS:
        if reused and label == "D0_baseline": continue
    ...
    print("\n\n" + "="*100)
    print(f"  DRAGON-TIGER + REPURCHASE A/B — {START} → {END}")
    print("="*100)
    print(f"{'Variant':<22} {'DTwt':>5} {'REPwt':>5} {'Return%':>9} {'MaxDD%':>8} {'Sharpe':>8} {'Trades':>7}")
    print("-"*100)
    for r in valid:
        marker = " 🏆" if r == valid[0] else ""
        print(f"{r['label']:<22} {r['dt_weight']:>5.2f} {r['rep_weight']:>5.2f} {r['return_pct']:>+8.2f}% {r['max_drawdown_pct']:>7.2f}% {r['sharpe']:>+8.3f} {r['n_trades']:>7}{marker}")
    print("="*100)
    with open("logs/dt_rep_results.json", "w") as f:
