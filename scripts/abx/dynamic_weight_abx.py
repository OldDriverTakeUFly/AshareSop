"""A/B: dynamic position weighting vs equal-weight.

  W0_equal (reference) — equal weight 1/max_positions (current default)
  W1_dynamic           — score-weighted (high score = larger position)
"""
...
VARIANTS = [
    ("W0_equal", {**BASE, "enable_dynamic_weight": False}),
    ("W1_dynamic", {**BASE, "enable_dynamic_weight": True}),
]
...
def run_variant(label, params, universe):
    print(f"\n{'='*70}\n  {label}: dynamic_weight={params.get('enable_dynamic_weight', False)}\n{'='*70}", flush=True)
    account = reset_account(f"dw_{label}", params)
...
def main():
    ...
    print("\n\n" + "="*90)
    print(f"  DYNAMIC WEIGHT A/B — {START} → {END}")
    print("="*90)
    print(f"{'Variant':<20} {'Return%':>9} {'MaxDD%':>8} {'Sharpe':>8} {'Trades':>7}")
    print("-"*90)
    for r in sorted(results, key=lambda x: -x["sharpe"]):
        print(f"{r['label']:<20} {r['return_pct']:>+8.2f}% {r['max_drawdown_pct']:>7.2f}% {r['sharpe']:>+8.3f} {r['n_trades']:>7}")
    print("="*90)
    with open("logs/dynamic_weight_results.json", "w") as f:
