"""A/B: intraday gap factor weight sweep.

  G0_baseline (reference) — gap_weight=0
  G1_gap_05              — gap_weight=0.05
  G2_gap_10              — gap_weight=0.10
"""
...
VARIANTS = [
    ("G0_baseline", {**BASE, "gap_weight": 0.0}),
    ("G1_gap_05", {**BASE, "gap_weight": 0.05}),
    ("G2_gap_10", {**BASE, "gap_weight": 0.10}),
]
...
def run_variant(label, params, universe):
    gw = params.get("gap_weight", 0)
    print(f"\n{'='*70}\n  {label}: gap_weight={gw}\n{'='*70}", flush=True)
    account = reset_account(f"gp_{label}", params)
    ...
    account.close()
    return {"label": label, "gap_weight": gw, "return_pct": ret,
...
def main():
    ...
    print("\n\n" + "="*95)
    print(f"  INTRADAY GAP A/B — {START} → {END}")
    print("="*95)
    print(f"{'Variant':<22} {'GapWt':>6} {'Return%':>9} {'MaxDD%':>8} {'Sharpe':>8} {'Trades':>7}")
    print("-"*95)
    for r in valid:
        marker = " 🏆" if r == valid[0] else ""
        print(f"{r['label']:<22} {r['gap_weight']:>6.2f} {r['return_pct']:>+8.2f}% {r['max_drawdown_pct']:>7.2f}% {r['sharpe']:>+8.3f} {r['n_trades']:>7}{marker}")
    print("="*95)
    with open("logs/gap_factor_results.json", "w") as f:
