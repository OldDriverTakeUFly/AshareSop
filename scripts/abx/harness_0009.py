"""随机窗口×随机票池截断 测试台 — 实验0009 专用副本 (变体可CLI选择).

基于 random_window_harness.py (0003 纪律, 同 SEED=2026 → 同一组 16 个 trial,
与 0003 历史结果直接可比). 差异: VARIANTS 换成 baseline + E1_pbstruct/E2_pbstruct
(回调结构门控卖出, 实验0009), 输出 logs/abx/harness_0009.json.
判读: 只看同 trial 配对差值 (E_x - baseline), 不看混合胜率 (0003 校准教训).
...
"""
import os, sys, time, json, sqlite3, random, argparse
import numpy as np
...
ALL_VARIANTS = [
    ("baseline", {}),
    ("E1_pbstruct", dict(pb_struct_highvol_exempt=True)),
    ("E2_pbstruct", dict(pb_struct_highvol_exempt=True, pb_struct_tplus_exempt=True)),
]
...
def main():
    ...
    ap.add_argument("--variants", default="baseline,E1_pbstruct,E2_pbstruct",
                    help="逗号分隔, 取自 ALL_VARIANTS; 只跑通过全期初筛的变体+baseline")
    args = ap.parse_args()
    chosen = [v for v in ALL_VARIANTS if v[0] in args.variants.split(",")]
    assert chosen, f"无效变体名: {args.variants}"
    global VARIANTS
    VARIANTS = chosen
    print(f"\n{'=' * 92}")
    ...
    with open("logs/abx/harness_0009.json", "w") as f:
    ...
    # 配对差值汇总 (0009: 只看同 trial 配对差, 不看混合胜率)
    b = VARIANTS[0][0]
    for name, _ in VARIANTS[1:]:
        diffs = [r[name]["ret"] - r[b]["ret"] for r in results]
        wins = sum(1 for d in diffs if d > 0)
        print(f"\n  配对 [{name} - {b}]: 胜 {wins}/{len(diffs)}, "
              f"均值 {np.mean(diffs):+.2f}pp, 中位 {np.median(diffs):+.2f}pp")
    print(f"\n  结果: logs/abx/harness_0009.json")
