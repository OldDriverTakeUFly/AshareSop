"""A/B: intraday amplitude filter.

  I0_baseline (reference) — max_intraday_amplitude=0 (no filter)
  I1_amp_06             — max_intraday_amplitude=0.06 (6% range = skip)
  I2_amp_08             — max_intraday_amplitude=0.08
  I3_amp_10             — max_intraday_amplitude=0.10
"""
...
VARIANTS = [
    ("I0_baseline", {**BASE, "max_intraday_amplitude": 0.0}),
    ("I1_amp_06", {**BASE, "max_intraday_amplitude": 0.06}),
    ("I2_amp_08", {**BASE, "max_intraday_amplitude": 0.08}),
    ("I3_amp_10", {**BASE, "max_intraday_amplitude": 0.10}),
]
...
def run_variant(label, params, universe):
    amp = params.get("max_intraday_amplitude", 0)
    print(f"\n{'='*70}\n  {label}: max_amplitude={amp}\n{'='*70}", flush=True)
    account = reset_account(f"ia_{label}", params)
    ...
    account.close()
    return {"label": label, "max_amplitude": amp, "return_pct": ret,
...
def main():
    ...
    results = []
    # Reuse I0 from production_final
    try:
        with sqlite3.connect(DB_PATH) as c:
            ...
            if row[0] >= 120:
                ...
                ret = (nav[-1]/INITIAL_CAPITAL-1)*100 if nav else 0
                mdd = max_dd(nav)
                results.append({"label": "I0_baseline (reused)", "max_amplitude": 0.0,
                ...
                account.close()
                print("  [reuse] I0 from production_final")
    except: pass
    reused = any("reused" in r["label"] for r in results)
    for label, params in VARIANTS:
        if reused and label == "I0_baseline": continue
    ...
    print("\n\n" + "="*95)
    print(f"  INTRADAY AMPLITUDE A/B — {START} → {END}")
    print("="*95)
    print(f"{'Variant':<22} {'MaxAmp':>7} {'Return%':>9} {'MaxDD%':>8} {'Sharpe':>8} {'Trades':>7}")
    print("-"*95)
    for r in valid:
        marker = " 🏆" if r == valid[0] else ""
        print(f"{r['label']:<22} {r['max_amplitude']:>7.2f} {r['return_pct']:>+8.2f}% {r['max_drawdown_pct']:>7.2f}% {r['sharpe']:>+8.3f} {r['n_trades']:>7}{marker}")
    print("="*95)
    with open("logs/intraday_amplitude_results.json", "w") as f:
