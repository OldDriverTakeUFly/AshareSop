"""A/B: 回调结构门控卖出 — 实验0009, 0008纪律的策略侧落地.

0008: 趋势中守 MA10(+69%续涨)/MA60 的良性回调不宜止盈, 卖飞代价中位+14~24%.
0004 用市场级豁免(bull+MA200)攻击同一病灶被 2022 否决(-12pp); 本次换成
个股级结构门控: 收盘>MA10 且 >MA60 时豁免高位放量/T+减仓(硬止损不动).

变体 (基线 = gx_G2_bull60 账户, 即当前生产, +126.40%/MDD15.6%/Sharpe1.081, 不重跑):
  E1_pbstruct — 结构完好时仅豁免高位放量卖出
  E2_pbstruct — 同时豁免高位放量 + T+减仓
判定线 (事前, 镜像0004):
  采纳: Sharpe>1.081 且 2024年度改善>=+3pp 且 无一年倒退>5pp 且 MDD<=17%
  否决: MDD>18% 或 2022倒退>5pp
  (全期通过后还须过随机窗口×票池测试台 16 trial 才可固化 — 双关标准)
烟测警示(2026-08-28): 924窗豁免生效(高位放量2→0,T+减仓2→0)但窗口收益
+2.71%→+0.85%; 窗口噪声大不作判定, 全期见分晓.
逐变体完成即落盘 (长回测运行规范#2).
"""
...
OUT_PATH = "logs/abx/pb_struct_exempt_abx.json"
...
def main():
    print(f"\n{'='*80}")
    print(f"  回调结构门控卖出 A/B — {START} → {END}  (基线=当前生产G2: +126.4%/1.081)")
    print(f"{'='*80}\n")
    ...
    print(f"\n  年度对比 (重点: 2024 是否修复>=+3pp, 2022 是否倒退>5pp):")
