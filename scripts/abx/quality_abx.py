"""A/B: Quality factor (QMJ-style) weight sweep.

  Q0_baseline (reference) — quality_weight=0
  Q1_quality_05           — quality_weight=0.05
  Q2_quality_10           — quality_weight=0.10
"""
...
VARIANTS = [
    ("Q0_baseline", {**BASE, "quality_weight": 0.0}),
    ("Q1_quality_05", {**BASE, "quality_weight": 0.05}),
    ("Q2_quality_10", {**BASE, "quality_weight": 0.10}),
]
...
def run_variant(label, params, universe):
    qw = params.get("quality_weight", 0)
    print(f"\n{'='*70}\n  {label}: quality_weight={qw}\n{'='*70}", flush=True)
    account = reset_account(f"qf_{label}", params)
    ...
    account.close()
    return {"label": label, "quality_weight": qw, "return_pct": ret,
...
def main():
    ...
    try:
        with sqlite3.connect(DB_PATH) as c:
            if account_nav_complete("production_amp08", START, END):
                ...
                ret = (nav[-1]/INITIAL_CAPITAL-1)*100 if nav else 0
                mdd = max_dd(nav)
                results.append({"label": "Q0_baseline (reused)", "quality_weight": 0.0,
                ...
                account.close()
                print("  [reuse] Q0 from production_amp08")
    except: pass
    reused = any("reused" in r["label"] for r in results)
    for label, params in VARIANTS:
        if reused and label == "Q0_baseline": continue
    ...
    print("\n\n" + "="*95)
    print(f"  QUALITY FACTOR A/B — {START} → {END}")
    print("="*95)
    print(f"{'Variant':<22} {'Qwt':>5} {'Return%':>9} {'MaxDD%':>8} {'Sharpe':>8} {'Trades':>7}")
    print("-"*95)
    for r in valid:
        marker = " 🏆" if r == valid[0] else ""
        print(f"{r['label']:<22} {r['quality_weight']:>5.2f} {r['return_pct']:>+8.2f}% {r['max_drawdown_pct']:>7.2f}% {r['sharpe']:>+8.3f} {r['n_trades']:>7}{marker}")
    print("="*95)
    with open("logs/quality_factor_results.json", "w") as f:
