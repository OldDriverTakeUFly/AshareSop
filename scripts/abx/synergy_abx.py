"""A/B: 动量×筹码共振因子的效果验证.
  ...
  H0_baseline    — 当前生产配置（无共振 boost）
  H1_synergy_05  — holder_momentum_synergy = 0.05
  H2_synergy_10  — holder_momentum_synergy = 0.10

实证依据（83433 stock-quarter 样本，2021-2025）:
  高动量+筹码集中 → +4.77% / 20d (胜率53%)
  高动量+筹码分散 → +1.24% / 20d (胜率42%)
  价差: +3.5pp

因子设计：holder_score × mom_score 的乘积项，只有双高才给大 boost。
"""
...
def run_variant(label, synergy_w, universe):
    print(f"\n{'='*70}\n  {label}: synergy={synergy_w}\n{'='*70}", flush=True)

    account = reset_account(f"syn_{label}")
    params = {**BASE, "holder_momentum_synergy": synergy_w}
    ...
    result = {
        "label": label, "synergy_weight": synergy_w,
...
def main():
    ...
    results = []
    results.append(run_variant("H0_baseline", 0.0, universe))
    results.append(run_variant("H1_synergy_05", 0.05, universe))
    results.append(run_variant("H2_synergy_10", 0.10, universe))

    print(f"\n\n{'='*90}")
    print(f"  共振因子 A/B 总览 — {START} → {END}")
    print(f"{'='*90}")
    print(f"  {'变体':<18} {'synergy':>8} {'净收益':>8} {'回撤':>6} {'Sharpe':>7} {'交易':>5}")
    for r in sorted(results, key=lambda x: -x["sharpe_real"]):
        print(f"  {r['label']:<18} {r['synergy_weight']:>8.2f} "
    ...
    out_path = "logs/synergy_abx.json"
