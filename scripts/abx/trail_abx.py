"""A/B: 跟踪止损 + 最小持仓 + 快速止损 的叠加效果.

  T0_baseline    — C3 生产配置（固定止盈，无跟踪止损/最小持仓/快速止损）
  T1_trail_only  — 仅跟踪止损（8%回撤，盈利10%激活）
  T2_all         — 跟踪止损 + 最小持仓5天 + 快速止损5%/5天
"""
...
def run_variant(label, extra, universe):
    print(f"\n{'='*70}\n  {label}\n{'='*70}", flush=True)
    account = reset_account(f"trail_{label}")
    params = {**BASE, **extra}
    ...
    for t in trades:
        if t.action == "SELL":
            ...
            if "跟踪止损" in r: sell_cats["跟踪止损"] += 1
            elif "快速止损" in r: sell_cats["快速止损"] += 1
            elif "硬止损" in r: sell_cats["硬止损"] += 1
            elif "止盈" in r: sell_cats["止盈"] += 1
            elif "T+减仓" in r: sell_cats["T+减仓"] += 1
            elif "超跌反弹" in r: sell_cats["超跌反弹卖"] += 1
            else: sell_cats["其他"] += 1
    ...
    account.close()
    result = {
        "label": label, "return_pct": round(total_ret, 2),
        "max_drawdown_pct": round(total_mdd, 2), "sharpe_real": round(sharpe_real, 3),
        "ann_vol_pct": round(ann_vol, 1), "n_trades": len(trades),
        "sell_cats": dict(sell_cats), "elapsed_hr": round(elapsed / 3600, 1),
        "annual_returns": {yr: round(r, 2) for yr, r in year_rets.items()},
    }
    print(f"  → {label}: 收益={total_ret:+.2f}% MDD={total_mdd:.1f}% "
          f"Sharpe={sharpe_real:+.3f} 波动={ann_vol:.1f}% ({elapsed/3600:.1f}h)", flush=True)
    print(f"    卖出: {dict(sell_cats)}", flush=True)
...
def main():
    print(f"\n{'='*80}")
    print(f"  跟踪止损+最小持仓+快速止损 A/B — {START} → {END}")
    print(f"{'='*80}\n")
    ...
    results = []
    results.append(run_variant("T0_baseline", {
        "trailing_drawback": 0.0, "min_hold_days": 0, "quick_stop_pct": 0.0,
    }, universe))
    results.append(run_variant("T1_trail_only", {
        "trailing_drawback": 0.08, "trailing_activate": 0.10,
        "min_hold_days": 0, "quick_stop_pct": 0.0,
    }, universe))
    results.append(run_variant("T2_all", {
        "trailing_drawback": 0.08, "trailing_activate": 0.10,
        "min_hold_days": 5, "quick_stop_pct": 0.05, "quick_stop_days": 5,
    }, universe))

    print(f"\n\n{'='*90}")
    print(f"  吃波段 A/B 总览")
    print(f"{'='*90}")
    print(f"  {'变体':<18} {'收益':>8} {'MDD':>6} {'Sharpe':>7} {'波动':>6} {'交易':>5}")
    for r in sorted(results, key=lambda x: -x["sharpe_real"]):
        print(f"  {r['label']:<18} {r['return_pct']:>+7.2f}% {r['max_drawdown_pct']:>5.1f}% "
              f"{r['sharpe_real']:>+7.3f} {r['ann_vol_pct']:>5.1f}% {r['n_trades']:>5}")
    ...
    out_path = "logs/abx/trail_abx.json"
