    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if not release_date or market_cap == 0:
                continue
            cap_display = (
                f"{market_cap / 1e8:.1f}亿" if market_cap >= 1e8 else f"{market_cap / 1e4:.1f}万"
            )
...
def main():
    ...
    parser.add_argument(
        "--add-event", action="append", default=[], help="Manual event name (repeatable)"
    )
    parser.add_argument("--sector", default="手动录入", help="Affected sector for manual events")
    parser.add_argument(
        "--severity", default=None, help="Severity emoji for manual events (auto-graded if omitted)"
    )
