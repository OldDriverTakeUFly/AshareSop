...
Runs the 4 stockhot analysis modules (limit_up → dragon_tiger +
fund_flow → risk_alert) in the order mandated by the daily-market-scan
skill, with try/except isolation so one module's failure does not block
others. Persists all results to SQLite.
...
# ── Wave 2: dragon_tiger + fund_flow (parallel-safe, neither reads the other) ──


def run_wave_2(trade_date: str) -> tuple[bool, bool]:
    """dragon_tiger (龙虎榜) + fund_flow (资金流)."""
    # fund_flow
    ff_ok = False
    try:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run daily market scan (4 modules)")
    ...
    ff_ok, dt_ok = run_wave_2(trade_date)
