...
class TestLimitUpIntegration:
    def test_full_pipeline_with_mock(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "stockhot.limit_up.ak.stock_zt_pool_zbgc_em", lambda date: make_broken_df()
        )
        monkeypatch.setattr(
            "stockhot.limit_up.ak.stock_zt_pool_dtgc_em", lambda date: make_limit_down_df()
        )
...
class TestDragonTigerIntegration:
    def test_full_pipeline_with_mock(self, monkeypatch):
        monkeypatch.setattr(
            "stockhot.dragon_tiger.ak.stock_lhb_detail_em", lambda **kw: make_lhb_df()
        )
        monkeypatch.setattr(
            "stockhot.dragon_tiger.ak.stock_lhb_jgmmtj_em", lambda **kw: make_institutional_df()
        )
        monkeypatch.setattr(
            "stockhot.dragon_tiger.ak.stock_lhb_hyyyb_em", lambda **kw: make_broker_df()
        )
...
class TestFundFlowIntegration:
    def test_full_pipeline_with_mock(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "stockhot.fund_flow.ak.stock_sector_fund_flow_rank",
            lambda **kw: make_sector_fund_flow_df(),
        )
...
class TestRiskAlertIntegration:
    def test_full_pipeline_with_mock(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "stockhot.risk_alert.get_daily_data", lambda *a, **kw: {"date": "2026-04-24"}
        )
